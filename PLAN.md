...
## 디자인
그린피스 공식 디자인 시스템(딥 네이비 + 라임그린 + 오렌지)을 기준으로 다듬음.
- 기본 배경: `#FFFFFF` · 글자: `#000000` · 카드: `#FFFFFF` · 테두리: `#E5E8E9`
- 1차 강조(버튼·CTA — "새 미팅 작성"·"로그인"·"저장하기"): 오렌지 `#FF8100`, 글자 `#FFFFFF`
- 2차 강조(링크·뱃지·사이드바 활성 표시): 라임그린 `#66CC00`
- 사이드바: 딥 네이비 `#292F47` 배경 + 흰 글자, 활성/호버 항목은 라임그린으로 표시
- 보조 색상: 회색 `#898B92`(보조 텍스트) · 옅은 회색 `#E5E8E9`(구분선) · 경고 `#FFBE00`
- 오류: `#BA1A1A`
- 폰트: Noto Sans KR (한글 가독성 우선)
- 모서리(radius): 버튼·인풋 8px · 카드 8px · 포커스 링: 라임그린
- 레이아웃 원칙: 좌측 사이드바 네비게이션(네이비 배경, 역할별 메뉴 다름, 활성 항목은 라임그린으로 표시) + 우측 콘텐츠 영역(표/카드). 대시보드는 상단 "다가오는 후속작업" 리스트 + 하단 최근 업무/개인미팅 요약 카드
